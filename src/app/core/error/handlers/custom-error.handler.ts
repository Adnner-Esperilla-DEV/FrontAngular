  ...
  handleError(error: any): void {
    ...
    switch (true) {
      ...
      case error instanceof HttpErrorResponse && error.status === 404:
        if (!environment.production) console.warn('Caught by custom 404-http-error handler', error.error)
        messages.push('El recurso solicitado no ha sido encontrado. Por favor, intente nuevamente.')
        break
