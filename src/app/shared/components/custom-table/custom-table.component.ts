import { AfterViewInit, Component, effect, ElementRef, inject, Input, input, model, output, signal, untracked, viewChild } from '@angular/core'
...
export class CustomTableComponent<T extends {}> implements AfterViewInit {
  ...
  codeProperty = input<keyof T>('id' as keyof T)
  ...
  @Input() actionRoute: string = 'detalle'; 
  get visibleColumns() {
    return this.columns()
  ...
  private searchTextEffect = effect((onCleanup) => {
    ...
    const timeout = setTimeout(() => this.searchByText.emit(searchTerm), 400)
  ...
  onRowClick(row: T) {
    ...
    const code = row[this.codeProperty()] as string
    this.router.navigate([code, this.actionRoute], { relativeTo: this.route })
  }
